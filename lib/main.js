...
const {convert} = require('./map');
const PyShell = require('python-shell');

var fs = require('browserify-fs');


$.getScript('../src-noconflict/ace.js', function () {
...
let handleEventStreamMessage = function (messageJson) {
    ...
    if (results.length > 0) {
        if (results[0].Alternatives.length > 0) {
            ...
            //$('#message').val(transcription + transcript + "\n");
...
let closeSocket = function () {
    $('#message').val($('#transcript').val());
    fs.mkdir('/home', function () {
        fs.writeFile('/home/trans.py', $('#transcript').val(), function () {
            fs.readFile('/home/trans.py', 'utf-8', function (err, data) {
                console.log(data);
            });
        });
    });
    editor.insert(convert($('#transcript').val()));
    //$('.ace_text-input').val($('#transcript').val());
    console.log($('.ace_text-input').val() + "ace");
...
$(document).ready(function() {
    ...
    $(document).on("click","#click_to_run_button",function() {
	    var myPyScript = '/home/trans.py';
	    var curPyShell = new PyShell(myPyScript);
	    
	    curPyShell.on("message", function(message) {
		    console.log(message);
	    });
	    curPyShell.end(function(err) {
		    if (err) {
			    throw err;
		    }
		    console.log("DONE");
